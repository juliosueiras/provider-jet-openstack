...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		openstackCreds := map[string]string{}
		if err := json.Unmarshal(data, &openstackCreds); err != nil {
		...
		ps.Env = []string{
			fmt.Sprintf("%s=%s", "OPENSTACK_USERNAME", openstackCreds["user_name"]),
			fmt.Sprintf("%s=%s", "OPENSTACK_PASSWORD", openstackCreds["password"]),
			fmt.Sprintf("%s=%s", "OPENSTACK_TENANT", openstackCreds["tenant_name"]),
